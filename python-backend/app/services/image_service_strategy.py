...
from app.schemas.image import ImageRequest, ImageData, DataType
...
class ImageServiceStrategy:
    ...
    async def get_image_and_upload(
        ...
        try:
            ...
            if (
                (not cos_url or not str(cos_url).strip())
                and image_data.data_type == DataType.URL
                and (image_data.url or "").strip()
            ):
                cos_url = (image_data.url or "").strip()
                logger.warning(
                    "COS 上传返回空，已改用原始图片 URL, method=%s",
                    method,
                )
            if cos_url and str(cos_url).strip():
                logger.info(f"图片获取并上传成功, method={method}, cosUrl={cos_url[:120]}...")
                return ImageResult(str(cos_url).strip(), method)
            else:
