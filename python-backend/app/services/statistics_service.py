...
# 扩展统计字段后 bump 版本，避免 Redis 里旧 JSON 缺字段
STATISTICS_CACHE_KEY = "statistics:overview:v2"
...
class StatisticsService:
    ...
    async def get_statistics(self) -> StatisticsVO:
        ...
        today_failed_count = await self._count_failed_articles_by_range(today_start, now)
    ...
    async def _count_failed_articles_by_range(self, start: datetime, end: datetime) -> int:
        """统计时间范围内 status=FAILED 的文章数（示例：今日失败数）。"""
        value = await self.db.fetch_val(
            query="""
                SELECT COUNT(1)
                FROM article
                WHERE isDelete = 0
                  ...
                  AND createTime <= :endTime
            """,
            values={"status": "FAILED", "startTime": start, "endTime": end},
        )
        return int(value or 0)
