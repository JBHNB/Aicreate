...
class CosService:
    """腾讯云 COS 服务（未配置 Region/密钥时跳过客户端，避免阻塞文章生成）"""

    def __init__(self):
        self.http_client = httpx.AsyncClient(timeout=30.0)
        self.region = (settings.tencent_cos_region or "").strip()
        self.bucket = (settings.tencent_cos_bucket or "").strip()
        sid = (settings.tencent_cos_secret_id or "").strip()
        sk = (settings.tencent_cos_secret_key or "").strip()

        if not (self.region and self.bucket and sid and sk):
            self.client = None
            logger.warning(
                "腾讯云 COS 未完整配置（region/bucket/secret），跳过 COS 客户端；配图将使用原始 URL"
            )
            return
    ...
    async def upload_image_data(
        ...
        if not self.client:
            ...
            if image_data.data_type == DataType.DATA_URL and image_data.url:
                return image_data.url
            if image_data.data_type == DataType.BYTES and image_data.bytes:
                # 无 COS 时退回 data URL，正文里仍可内嵌显示（体积会增大）
                mt = image_data.mime_type or "image/png"
                b64 = base64.standard_b64encode(image_data.bytes).decode("ascii")
                data_url = f"data:{mt};base64,{b64}"
                logger.info(
                    "COS 未配置，将 BYTES 配图转为 data URL（长度=%s）",
                    len(data_url),
                )
                return data_url
            logger.warning("COS 未配置且图片数据无法转为可嵌入形式，返回 None")
            return None
