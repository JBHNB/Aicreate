...
class ArticleService:
    ...
    async def confirm_title(
        ...
        self._check_article_permission(article, login_user)
        throw_if(
            article["phase"] not in (
                ArticlePhaseEnum.TITLE_SELECTING.value,
                ArticlePhaseEnum.OUTLINE_GENERATING.value,
                ArticlePhaseEnum.OUTLINE_EDITING.value,
            ),
            ErrorCode.OPERATION_ERROR,
            "当前阶段不允许确认标题",
        )

        if article["phase"] == ArticlePhaseEnum.TITLE_SELECTING.value:
            await self.db.execute(
                query="""
                    UPDATE article
    ...
    async def save_article_content(self, task_id: str, state: ArticleState):
        ...
        # 未迁移前 coverImage 常为 varchar(512)：data URL 必超长，避免整单保存失败
        if cover_image and cover_image.startswith("data:") and len(cover_image) > 500:
            logger.warning(
                "封面为 data URL 长度=%s，超过常见 coverImage(varchar512) 限制，"
                "已置空 coverImage 列（配图仍在 images JSON 与 fullContent 中）。"
                "请执行 sql/migrate_article_content_longtext.sql 将 coverImage 改为 TEXT。",
                len(cover_image),
            )
            cover_image = None
        if cover_image and len(cover_image) > 65530:
            logger.warning("封面 URL 超过 MySQL TEXT 上限，已置空 coverImage")
            cover_image = None

        content = state.content
        full_content = state.full_content
        images_json = (
            json.dumps([img.model_dump(by_alias=True) for img in state.images], ensure_ascii=False)
            if state.images
            else None
        )
        try:
            ...
            logger.error(
                "保存文章内容失败（常见原因：① content/fullContent 仍为 TEXT(64KB) 请执行 "
                "sql/migrate_article_content_longtext.sql；② coverImage 仍为 varchar(512)，"
                "data URL 封面请执行同一脚本将 coverImage 改为 TEXT，或由程序已自动省略封面列）: %s",
                e,
                exc_info=True,
            )
            raise
