...
**更完整的分阶段开发 + 面试版：** [完整开发手册-从零到面试.md](./完整开发手册-从零到面试.md)  
...
### 12.4 测试 VIP 支付时要开的服务

除 MySQL、Redis、后端、前端外，**还必须**单独开一个终端跑 Stripe CLI，否则付完款不会自动升 VIP：
...
将终端输出的 `whsec_...` 写入 `python-backend/.env` 的 `STRIPE_WEBHOOK_SECRET`，并重启后端。  
详见：[完整开发手册 §8.2](./完整开发手册-从零到面试.md#82-本地要不要启动-stripe要才能自动升-vip) | [Stripe 实战总结](./2026-06-01-Git与Stripe实战总结.md)
...
| 付了 VIP 仍显示「升级 VIP」 | `stripe listen` 未开 / whsec 未配 / 后端未重启 | 开 listen；更新 `.env`；重启 uvicorn；刷新或重登 |
| `stripe` 命令找不到 | PATH 未生效 | 用 `D:\stripe_1.42.1_windows_x86_64\stripe.exe` 完整路径；重启 Cursor |
...
| [2026-06-01-Git与Stripe实战总结.md](./2026-06-01-Git与Stripe实战总结.md) | GitHub 上传、Git 回滚、Stripe VIP 支付与踩坑 |
| [**完整开发手册-从零到面试.md**](./完整开发手册-从零到面试.md) | **总手册：骨架搭建→分阶段开发→全文件索引→面试问答** |
