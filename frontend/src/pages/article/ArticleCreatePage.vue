    ...
    <div class="create-layout">
      ...
      <main ref="mainContentRef" class="main-content">
        ...
        <Transition name="fade-slide" mode="out-in">
          ...
          <div v-else-if="currentPhase === 'OUTLINE_PENDING'" key="outline-pending" class="loading-stage">
            <a-spin size="large" />
            <h3>已确认标题，正在生成大纲…</h3>
            <a-button style="margin-top: 16px" @click="resumeTask">继续生成大纲</a-button>
          </div>
          ...
          <div v-if="showPhase3Resume" class="resume-box">
            <a-alert type="warning" show-icon message="正文生成似乎中断，可尝试继续生成（勿频繁点击）" />
            <a-button type="primary" style="margin-top: 12px" @click="resumeTask">继续生成</a-button>
          </div>
          </div>
...
import { createArticle, confirmTitle, confirmOutline, getArticle, resumeArticle } from '@/api/article'
...
const DEFAULT_IMAGE_METHODS = ['PEXELS', 'MERMAID', 'ICONIFY', 'EMOJI_PACK']

// 智能体步骤（含 Reviewer，对齐 Python 流水线）
const agentSteps = [
  { title: '生成标题', description: 'AI 分析选题，生成吸睛标题' },
  { title: '规划大纲', description: '构建文章结构，理清脉络' },
  { title: '撰写正文', description: '流式生成高质量文章内容' },
  { title: '内容审核', description: '主编把关正文质量' },
  { title: '分析配图', description: '智能分析配图需求和位置' },
  { title: '生成配图', description: '自动匹配或 AI 生成配图' },
...
const selectedImageMethods = ref<string[]>([...DEFAULT_IMAGE_METHODS])
...
let phase3ResumeTimer: ReturnType<typeof setTimeout> | null = null
let lastSseTaskId = ''
const showPhase3Resume = ref(false)
...
function clearPhase3ResumeTimer() {
  if (phase3ResumeTimer) {
    clearTimeout(phase3ResumeTimer)
    phase3ResumeTimer = null
  }
}
...
function ensureSSE(tid: string, opts?: { force?: boolean }) {
  if (!opts?.force && eventSource && eventSource.readyState === EventSource.OPEN && lastSseTaskId === tid) {
    return
  }
  lastSseTaskId = tid
  closeSSE(eventSource)
  eventSource = connectSSE(tid, {
...
function startProgressPoll(tid: string) {
  stopStatusPoll()
  statusPollTimer = setInterval(async () => {
    if (!tid || currentPhase.value === 'COMPLETED') {
      stopStatusPoll()
      return
    }
    try {
      const art = await getArticle(tid)
      if (art.status === 'FAILED') {
        errorMessage.value = art.errorMessage || '创作失败'
        errorVisible.value = true
        isCreating.value = false
        stopStatusPoll()
        return
      }
      if (art.mainTitle) article.value.mainTitle = art.mainTitle
      if (art.subTitle) article.value.subTitle = art.subTitle
      if (art.phase === 'TITLE_SELECTING' && art.titleOptions?.length) {
        if (currentPhase.value === 'INPUT' || currentPhase.value === 'TITLE_GENERATING') {
          handleSSEMessage({ type: 'TITLES_GENERATED', titleOptions: art.titleOptions })
        }
        return
      }
      if (art.phase === 'OUTLINE_EDITING' && art.outline?.length) {
        if (currentPhase.value !== 'OUTLINE_EDITING') {
          handleSSEMessage({ type: 'OUTLINE_GENERATED', outline: art.outline })
        }
        return
      }
      if (art.phase === 'CONTENT_GENERATING') {
        ...
        if (art.content) {
          article.value.content = art.content
          isStreaming.value = false
        }
        if (art.fullContent) article.value.fullContent = art.fullContent
      }
      if (art.status === 'COMPLETED') {
        if (art.fullContent) article.value.fullContent = art.fullContent
        handleSSEMessage({ type: 'ALL_COMPLETE', taskId: tid })
      }
    } catch {
      /* ignore */
    }
...
function schedulePhase3AutoResume() {
  clearPhase3ResumeTimer()
  phase3ResumeTimer = setTimeout(() => {
    if (
      currentPhase.value !== 'CONTENT_GENERATING' ||
    ...
    showPhase3Resume.value = true
    addLog('长时间未收到正文，可点「继续生成」', 'error')
  }, 25000)
}
...
async function resumeTask() {
  ...
  try {
    const msg = await resumeArticle(taskId.value, true)
    addLog(msg, 'info')
    showPhase3Resume.value = false
    if (!msg.includes('正在执行中')) {
      ensureSSE(taskId.value, { force: true })
    }
  } catch (e) {
...
const startCreate = async () => {
  ...
  try {
    // 创建任务
    const newTaskId = await createArticle({
      topic: topic.value,
    ...
    ensureSSE(taskId.value, { force: true })
    startProgressPoll(taskId.value)
...
const handleSSEMessage = (msg: SSEMessage) => {
  ...
  switch (msg.type) {
    ...
    case 'TITLES_GENERATED':
      ...
      titleOptions.value = (msg.titleOptions as Array<{ mainTitle: string; subTitle: string }>) || []
      isCreating.value = false
      addLog(`生成了 ${titleOptions.value.length} 个标题方案`, 'success')
      break
    ...
    case 'OUTLINE_GENERATED':
      ...
      outline.value = (msg.outline as Array<{ section: number; title: string; points: string[] }>) || []
      isCreating.value = false
    ...
    case 'AGENT3_STREAMING':
      ...
      article.value.content = (article.value.content || '') + String(msg.content || '')
      scrollToBottom()
      break
    ...
    case 'AGENT_REVIEWER_COMPLETE': {
      const score = msg.reviewScore
      const passed = msg.reviewPassed
      currentStep.value = 3
      addLog(`内容审核完成（${passed ? '通过' : '已优化'}，${score ?? '-'} 分）`, 'success')
      break
    }
    ...
    case 'PHASE2_STARTED':
      currentPhase.value = 'OUTLINE_GENERATING'
      isCreating.value = true
      addLog('后端阶段2已启动', 'success')
      break

    case 'PHASE3_STARTED':
      clearPhase3ResumeTimer()
      showPhase3Resume.value = false
      addLog('后端阶段3已启动', 'success')
      break

    case 'AGENT4_COMPLETE':
      currentStep.value = 4
      totalImages.value = Array.isArray(msg.imageRequirements)
        ? msg.imageRequirements.length
        : 5
    ...
    case 'AGENT5_COMPLETE':
      // 所有配图完成，进入图文合成步骤
      currentStep.value = 5
      article.value.images = msg.images as ArticleVO['images']
      addLog('所有配图生成完成', 'success')
      break

    case 'MERGE_COMPLETE':
      // 图文合成完成
      article.value.fullContent = String(msg.fullContent ?? '')
      scrollToBottom()
    ...
    case 'ERROR':
      errorMessage.value = String(msg.message || '创作失败')
      errorVisible.value = true
      isCreating.value = false
...
const handleConfirmTitle = async (data: {mainTitle: string, subTitle: string, userDescription: string}) => {
  ...
  try {
    ...
    currentPhase.value = 'OUTLINE_PENDING'
    outlineRaw.value = ''
    isCreating.value = true
    startProgressPoll(taskId.value)
...
const resetCreate = () => {
  ...
  topic.value = ''
  selectedStyle.value = ''
  selectedImageMethods.value = [...DEFAULT_IMAGE_METHODS]
  titleOptions.value = []
  outline.value = []
  isCreating.value = false
